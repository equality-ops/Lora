...
void lora3a22_uart_callback(uint8 uart_dat)
{
    ...
    lora3a22_uart_data[length++] = uart_dat;
    ...
    if(LORA3A22_DATA_LEN <= length)                            	  // 数据长度判断
    {
        ...
        if (parity_bit_sum == parity_bit)                          // 和校验判断
        {
			...
			// 大小端不一样，需要交换
			temp[0] = lora3a22_uart_data[5];
			temp[1] = lora3a22_uart_data[4];
			temp[2] = lora3a22_uart_data[7];
			temp[3] = lora3a22_uart_data[6];
			temp[4] = lora3a22_uart_data[9];
			temp[5] = lora3a22_uart_data[8];
			temp[6] = lora3a22_uart_data[11];
			temp[7] = lora3a22_uart_data[10];
			
            // 将接收到的数据拷贝到结构体中
            memcpy(((uint8*)&lora3a22_uart_transfer), (uint8*)lora3a22_uart_data, sizeof(lora3a22_uart_data) - 8);
...
{
	// 设置串口中断回调函数
    set_wireless_type(LORA3A22_UART, LORA3A22_UART_INDEX, lora3a22_uart_callback);
